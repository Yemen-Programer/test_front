...
const NorthernRegionPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRegionData = async () => {
      try {
        setLoading(true);
        console.log('🔄 جلب بيانات المنطقة...');
        ...
        if (response.success) {
          ...
          console.log('✅ تم تحميل بيانات المنطقة بنجاح');
        } else {
          throw new Error(response.message || 'فشل في جلب البيانات');
        }
      } catch (err) {
        console.error('❌ خطأ في جلب بيانات المنطقة:', err);
        setError('فشل في تحميل بيانات المنطقة يرجى تسجيل الدخول');
      } finally {
        setLoading(false);
      }
    };
...
// دالة خاصة لعرض بطاقات الملابس بنفس تصميم المنطقة الغربية
