...
class VoteService {
  ...
  async getContentVotes(contentId) {
    try {
      const response = await fetch(`${API_BASE_URL}/votes/content/${contentId}`);
  ...
  async checkUserVote(userId, contentId) {
    try {
      const response = await fetch(`${API_BASE_URL}/votes/check/${userId}/${contentId}`);
  ...
  async getVotesStatus(userId, contentIds) {
    try {
      const response = await fetch(`${API_BASE_URL}/votes/status/${userId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
