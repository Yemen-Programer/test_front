...
const WesternRegionPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRegionData = async () => {
      try {
        setLoading(true);
        console.log('🔄 جلب بيانات المنطقة الغربية...');
        
        const response = await ContentService.getRegionStructure('western');
        ...
        if (response.success) {
          ...
          console.log('✅ تم تحميل بيانات المنطقة الغربية بنجاح');
        } else {
  ...
  const createGoogleMapsUrl = (item: ContentItem): string => {
    ...
    // إذا كان العنوان معروفاً مسبقاً
    const knownSites = {
      "حي البلد": "21.4858,39.1925",
      "سوق البلد": "21.4858,39.1925",
      "منطقة جدة التاريخية": "21.4858,39.1925",
      "قصر خزام": "21.4785,39.1678"
    };
    ...
    return `https://maps.google.com/?q=${encodeURIComponent(item.title + ' المنطقة الغربية')}`;
  };
  ...
  // بيانات افتراضية للمنطقة الغربية
    ...
    <div className="region-page">
      ...
      <header className="region-header">
        <div className="header-content pt-20">
          <h1 className="region-title">المنطقة الغربية</h1>
          <p className="region-description">عد المنطقة الغربية (الحجاز) قلب المملكة الديني والثقافي، حيث تحتضن أقدس المقدسات في الإسلام: الحرمين الشريفين في مكة المكرمة والمدينة المنورة. تتميز بتراثها العريق الذي يتجلى في أسواقها التقليدية كسوق البلد في جدة، وعاداتها الأصيلة مثل رقصة الخبيتي، مما يعكس هويتها الثقافية الغنية. تجمع المنطقة بين قدسية التاريخ وحيوية الحاضر، من خلال مدنها الرئيسية مثل جدة والطائف التي تمزج بين الأصالة والحداثة. تبقى هذه المنطقة رمزًا للتنوع الثقافي والعمق التاريخي في المملكة، وشاهدًا حيًا على امتزاج التراث بالإبداع المعاصر</p>
        </div>
      </header>
