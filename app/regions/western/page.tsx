    ...
    <div className="region-page">
      ...
      <header className="region-header">
        <div className="header-content pt-20">
          ...
          <p className="region-description">تُعد المنطقة الغربية (الحجاز) قلب المملكة الديني والثقافي، حيث تحتضن أقدس المقدسات في الإسلام: الحرمين الشريفين في مكة المكرمة والمدينة المنورة.
تتميز بتراثها العريق الذي يتجلى في أسواقها التقليدية كسوق البلد في جدة، وعاداتها الأصيلة مثل رقصة الخبيتي، مما يعكس هويتها الثقافية الغنية.
تجمع المنطقة بين قدسية التاريخ وحيوية الحاضر، من خلال مدنها الرئيسية مثل جدة والطائف التي تمزج بين الأصالة والحداثة.
تبقى هذه المنطقة رمزًا للتنوع الثقافي والعمق التاريخي في المملكة، وشاهدًا حيًا على امتزاج  بالتراث والإبداع المعاصر</p>
        </div>
      </header>
            ...
            {!selectedCategory ? (
              ...
              <>
                <div className="category-header">
                  ...
                  <h2>{clothingCategories.find(cat => cat.id === selectedCategory)?.name}</h2>
                </div>
                <div className="items-grid">
                  {data.clothing[selectedCategory as keyof typeof data.clothing]?.map((item: ContentItem, index: number) => (
                    <div key={item.id || index} className="item-card">
                      <div className="item-image">
                        {item.imageUrl ? (
                          <img 
