...
// تعريف الأنواع TypeScript المحدثة
...
const EasternRegionPage: React.FC = () => {
  ...
  // دالة لمعالجة URLs المحتوى
  const processContentUrls = (data: any): RegionData => {
    const processItems = (items: ContentItem[]): ContentItem[] => {
      return items.map(item => ({
        ...
        model3durl: item.model3durl 
          ? `${API_BASE_URL}/uploads/${item.model3durl}`
          : null
      }));
    };
  ...
  useEffect(() => {
    const fetchRegionData = async () => {
      try {
        setLoading(true);
        console.log('🔄 جلب بيانات المنطقة الشرقية...');
        
        const response = await ContentService.getRegionStructure('eastern');
        ...
        if (response.success) {
          ...
          console.log('✅ تم تحميل بيانات المنطقة الشرقية بنجاح');
        } else {
  ...
  // دالة لإنشاء روابط خرائط Google
  const createGoogleMapsUrl = (item: ContentItem): string => {
    // إذا كان الرابط موجوداً بالفعل
    if (item.googlemapsurl) {
      return item.googlemapsurl;
    }
    
    // إذا كانت الإحداثيات متوفرة
    if (item.coordinates) {
      const [lat, lng] = item.coordinates.split(',').map(coord => coord.trim());
      return `https://maps.google.com/?q=${lat},${lng}`;
    }
    
    // إذا كان العنوان معروفاً مسبقاً للمنطقة الشرقية
    const knownSites = {
      "واحة الأحساء": "25.3800,49.5881",
      "قصر إبراهيم": "26.4362,50.1031",
      "متحف الأحساء": "25.3800,49.5881",
      "جبل القارة": "25.4167,49.6833"
    };
    
    if (knownSites[item.title]) {
      const [lat, lng] = knownSites[item.title].split(',');
      return `https://maps.google.com/?q=${lat},${lng}`;
    }
    
    // رابط افتراضي بالبحث عن العنوان
    return `https://maps.google.com/?q=${encodeURIComponent(item.title + ' المنطقة الشرقية السعودية')}`;
  };
  ...
  const renderItemCard = (item: ContentItem, showVoteButton: boolean = true, showWishlistButton: boolean = true) => (
    <div key={item.id} className="item-card">
      <a href={createGoogleMapsUrl(item)} target="_blank" rel="noopener noreferrer">
        ...
        <div className="card-actions">
          ...
          {showWishlistButton && activeTab === 'heritage' && (
            <button 
              ...
              title={wishlistedContents.has(item.id) ? 'إزالة من قائمة الأمنيات' : 'إضافة إلى قائمة الأمنيات'}
            >
  ...
  // بيانات افتراضية للمنطقة الشرقية
    ...
    <div className="region-page">
      ...
      <header className="region-header">
        <div className="header-content pt-20">
          <h1 className="region-title">المنطقة الشرقية</h1>
          <p className="region-description">تعد المنطقة الشرقية من أهم مناطق المملكة العربية السعودية، حيث تقع على ساحل الخليج العربي وتضم مدناً رئيسية كالدمام والخبر والظهران، مما يجعلها قلباً اقتصادياً وتجارياً مهماً. تمتاز بتراث غني وواحة الأحساء، إحدى أكبر الواحات الزراعية في العالم، التي تعكس تاريخ المنطقة الحضاري. كما تبرز فيها مظاهر الثقافة الأصيلة من خلال الأهازيج الشعبية والعادات والتقاليد المتوارثة. تجمع المنطقة بين عبق الماضي وحيوية الحاضر، لتبقى رمزاً للهوية السعودية وجسراً يربط التراث بالتنمية الحديثة.</p>
        </div>
      </header>
...
export default EasternRegionPage;
