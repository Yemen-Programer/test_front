...
const SouthernRegionPage: React.FC = () => {
    ...
    const getUserFromStorage = () => {
      try {
        ...
        if (userData) {
          ...
          console.log('👤 المستخدم الم parsed:', user);
  ...
  useEffect(() => {
    const fetchRegionData = async () => {
      try {
        setLoading(true);
        console.log('🔄 جلب بيانات المنطقة الجنوبية...');
        ...
        if (response.success) {
          ...
          console.log('✅ تم تحميل بيانات المنطقة الجنوبية بنجاح');
        } else {
  ...
  const createGoogleMapsUrl = (item: ContentItem): string => {
    ...
    // إذا كان العنوان معروفاً مسبقاً للمنطقة الجنوبية
    const knownSites = {
      "قرية رجال ألمع التراثية": "18.2167,42.5044",
      "سوق الثلاثاء": "18.2391,42.5059",
      "جبال السودة": "18.2500,42.4167",
      "وادي لجب": "18.3333,42.3333"
    };
    ...
    return `https://maps.google.com/?q=${encodeURIComponent(item.title + ' المنطقة الجنوبية السعودية')}`;
  };
        ...
        <div className="card-actions">
          ...
          {!votedContents.has(item.id) && (
            <button 
              ...
              title={currentUserId ? 'انقر للتصويت' : 'يجب تسجيل الدخول للتصويت'}
            >
              <Heart size={20} fill="none" color="gray" />
              <span> احببته ({item.votesCount})</span>
            </button>
  ...
  // بيانات افتراضية للمنطقة الجنوبية
  const defaultRegionData: RegionData = {
    name: 'المنطقة الجنوبية',
    description: 'تُعد المنطقة الجنوبية من أروع مناطق المملكة العربية السعودية، إذ تجمع بين الطبيعة الخلابة والتراث العريق والثقافة الغنية. تقع في الجزء الجنوبي الغربي من المملكة، وتشمل مناطق عسير، الباحة، جازان، ونجران، وتمتد بين الجبال المرتفعة والوديان الخصبة والسواحل الخلابة، ما يمنحها تنوعًا جغرافيًا فريدًا يجعلها منطقة سياحية وثقافية مهمة.',
    heritageSites: [],
    intangibleHeritage: {
      oral: [],
      folklore: [],
    ...
    <div className="region-page">
      ...
      <header className="region-header">
        <div className="header-content pt-20">
          <h1 className="region-title">المنطقة الجنوبية</h1>
          <p className="region-description">
            تُعد المنطقة الجنوبية من أروع مناطق المملكة العربية السعودية، إذ تجمع بين الطبيعة الخلابة والتراث العريق والثقافة الغنية. تقع في الجزء الجنوبي الغربي من المملكة، وتشمل مناطق عسير، الباحة، جازان، ونجران، وتمتد بين الجبال المرتفعة والوديان الخصبة والسواحل الخلابة، ما يمنحها تنوعًا جغرافيًا فريدًا يجعلها منطقة سياحية وثقافية مهمة.
          </p>
        </div>
      </header>
...
export default SouthernRegionPage;
